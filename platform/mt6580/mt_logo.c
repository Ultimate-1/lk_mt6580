 ...
 *
 */
void mt_disp_show_menu(int number)
{
    dprintf(0, "[lk logo: %s %d]\n",__FUNCTION__,__LINE__);
    mt_logo_get_custom_if();
    ...
    else
    {
        ...
        switch(number) {
            case 1:
                fill_animation_logo(BOOT_MENU_1, mt_get_fb_addr(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
            break;

            case 2:
                fill_animation_logo(BOOT_MENU_2, mt_get_fb_addr(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
            break;

            case 3:
                fill_animation_logo(BOOT_MENU_3, mt_get_fb_addr(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
            break;

            case 4:
                fill_animation_logo(BOOT_MENU_4, mt_get_fb_addr(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
            break;
        }
        mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
    }
