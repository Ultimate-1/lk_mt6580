...
__attribute__ ((aligned(64))) static unsigned char g_data_buf[16384+1280];
...
__attribute__ ((aligned(64))) static unsigned char oob_buf_temp[1280];
...
#endif
{
	...
	dprintf(INFO,"[nand_wite_img]write to img size, %x addr %lx\n",img_sz,addr);
	...
	if(img_type == YFFS2_IMG){
		...
		if(img_sz % write_size){
			dprintf(INFO,"[nand_write_img]img size is not w_size %d alignment\n",write_size);
			return -1;
		}
	}else{
