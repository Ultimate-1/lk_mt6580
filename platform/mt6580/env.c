...
#else
	len = dev->write(dev, (uchar*)env_buf, start_addr + mtk_nand_erasesize() , CFG_ENV_SIZE, 0);	 
#endif
	if (len < 0) {
		return -EIO;
	}
...
#else
	len = dev->read(dev, start_addr + mtk_nand_erasesize(), (uchar*)env_buf, CFG_ENV_SIZE, 0);    
#endif
    if (len < 0) {
		return -EIO;
    }
